...
public final class TabSyncService {
    ...
    public boolean refreshSubjectDisplay(Player subject) {
        ...
        for (Player viewer : proxy.getAllPlayers()) {
            TabList tab = viewer.getTabList(); Map<UUID, Component> styled = decorated.computeIfAbsent(viewer.getUniqueId(), ignored -> new ConcurrentHashMap<>());
            var existing = tab.getEntry(uuid); if (existing.isPresent()) { applyDisplay(existing.get(), styled, uuid, display); continue; }
            if (ChatBridgeService.serverName(viewer).equalsIgnoreCase(ChatBridgeService.serverName(subject))) {
                styled.remove(uuid); if (viewer.getUniqueId().equals(uuid)) selfReady = false; continue;
            }
            Set<UUID> ours = managed.computeIfAbsent(viewer.getUniqueId(), ignored -> ConcurrentHashMap.newKeySet()); refreshEntry(viewer, subject, tab, ours, styled);
        }
        return selfReady;
    }

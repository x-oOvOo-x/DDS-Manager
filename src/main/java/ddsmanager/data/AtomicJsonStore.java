...
public final class AtomicJsonStore {
    private final Gson gson; private final Logger logger;
    ...
    public synchronized <T> Optional<T> read(Path path, Class<T> type) throws IOException { return read(path, type, UnaryOperator.identity()); }
    public synchronized <T> Optional<T> read(Path path, Class<T> type, UnaryOperator<String> filter) throws IOException {
        Path backup = sibling(path, ".bak");
        if (!Files.exists(path)) {
            if (!Files.exists(backup)) return Optional.empty();
            T value = readRequired(backup, type, filter); restore(path, backup, false); return Optional.of(value);
        }
        try { return Optional.of(readRequired(path, type, filter)); }
        catch (IOException primary) {
            if (!Files.exists(backup)) throw primary;
            T value; try { value = readRequired(backup, type, filter); }
    ...
    public synchronized void write(Path path, Object value) throws IOException { writeText(path, gson.toJson(value)); }
    public synchronized void writeText(Path path, String text) throws IOException {
        ...
        if (Files.exists(path)) Files.copy(path, backup, StandardCopyOption.REPLACE_EXISTING); move(tmp, path);
    }

    private <T> T readRequired(Path path, Class<T> type, UnaryOperator<String> filter) throws IOException {
        try {
            T value = gson.fromJson(filter.apply(Files.readString(path, StandardCharsets.UTF_8)), type);
            if (value == null) throw new IOException("JSON document is null: " + path.getFileName()); return value;
    ...
    private void restore(Path path, Path backup, boolean quarantine) throws IOException {
        ...
        move(tmp, path);
        if (logger != null) logger.warn("Recovered {} from backup{}", path, quarantine ? "; damaged file kept as " + corrupt.getFileName() : "");
    }
